...
historyHelp = (event) => {
    ...
    $('#helpText').html("SOUTH ATLANTIC TIMES, 3 MAY 2019 <br />  <br /> Rumours of a fabled treasure - The Lost Scroll to Jubilee, is currently trending in The Krappacino Forums. Most of the penguinfolk remain skeptical, except for a bunch of individualistic teenagers. They managed to find only one yellow submarine and agreed the treasure to be finder's keeper. <br /> <br /> The submarine is pivotal in their campaign to find the fabled treasures (if any). However, the submarine has a limited supply of fresh air - to be shared among the young Krappas. The outcome of the expedition will depend on the individual choices of these young penguins; will it be known as The Abyss-Adventure or A Mis-Adventure? <br /><br /> Click buttons below for more information")
}
turnHelp = (event) => {
    clickSound.play()
    $('.chosenHelp').attr('class', 'helpBtn')
    $(event.currentTarget).attr('class','chosenHelp')
    $('#helpText').html("BREATHE: At the start of each player's turn, the Submarine's air supply will be reduced by the number of treasures a player is holding. If the air supply reaches 0, the current player's turn will be the last one of the entire round. <br /> <br /> DIVE or RETURN: Player will roll the dice and move tiles equal to the roll. If player is carrying treasure, subtract the number of treasure from the roll. Eg. If player is carrying 3 treasures & rolls a 4, player will move by 1 tile only. Player can skip over tiles that are already occupied by another player. <br />Only once a player has picked his first treasure, player can decide to return back to submarine or continue diving deeper. Once a player decides to return, he must continue all the way back to submarine for that round. <br /><br /> TILE ACTIONS: Player can pick or drop treasure, or do nothing at arrived tile. Player can only drop treasure on empty tiles. Player will always first drop lowest tier of treasure. Turn ends after this.")
}
scoreHelp = (event) => {
    clickSound.play()
    $('.chosenHelp').attr('class', 'helpBtn')
